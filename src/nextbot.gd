...
func _ready():
	...
	# Автоматически добавляем в группу enemy, чтобы лазер точно сработал
...
func _physics_process(delta: float):
	...
	# Используем ПОЛНОЦЕННУЮ физику для перемещения
	move_and_slide()
...
# --- ФУНКЦИЯ ПОЛУЧЕНИЯ УРОНА (вызывается из скрипта игрока) ---
func take_damage(amount: float) -> void:
	health -= amount
	if health <= 0:
		queue_free() # Уничтожает некстобота при гибели
